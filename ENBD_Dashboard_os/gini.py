...
def app():
    st.markdown(
        ...
        <div style='background-color: #008080; border-radius: 5px; padding: 1px;'>
            <h1 style='text-align: center; font-size: 28px; color: white;'>PL - Scorecard Model Gini</h1>
        </div>
    ...
    # Read the Excel file
    df = pd.read_excel(os.path.join(base_dir, 'Datasets', 'Gini_Data_dashboard.xlsx'))  # Ensure the file is in the 'Datasets' directory
    ...
    # Read the Excel file
    df1 = pd.read_excel(os.path.join(base_dir, 'Datasets', 'Gini_Data_dashboard_dev.xlsx'))  # Ensure the file 'data.xlsx' is in the same directory as your script
    # Replace None values with empty strings for better display
    df1 = df1.fillna("")
    st.session_state.df_gini_dev = df1  # Save df to session_state
